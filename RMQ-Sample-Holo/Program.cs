...
namespace RMQ_Sample_Holo
{
    ...
    public class HelloWorldApplication : StereoApplication
    {
        ...
        protected override async void Start()
        {
            ...
            //initialize UI 2 Dimension
            ui = new UITwoDimension(ref uiRoot);

            //start rabbitMQ
            rabbitmq = new RMQ();
            rabbitmq.createChannel("testing", "amq.topic", "testing");
        ...
        protected override void OnUpdate(float timeStep)
        {
            ...
            if (rabbitmq.getData() != "")
            {
                try
                {
                    dataJson = JsonConvert.DeserializeObject<DataJSON>(rabbitmq.getData());
                    dataRMQ = rabbitmq.getData();
                    ui.updateUI(dataJson.x.ToString(), dataJson.y.ToString(), dataJson.z.ToString(), rabbitmq.getData());
                }
